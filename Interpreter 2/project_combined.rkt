...
(define mValueAll
  (lambda (expression state)
    (cond     
      ((null? expression) '())
      ;((and (not (number? expression)) (atom? expression)) (lookUpValue expression state))
      ...
      (else (lookUpValue expression state)))))
...
(define lookUpValue 
  (lambda (name state)
    (cond
      ...
      ((number? name) name)
      ((null? (nameList state)) '())
      ((eq? name (firstName state))  (firstValue state)) 
...
(define removeState
  (lambda (name state)
    (cond
      ...
      (else (appendState (obama state) (removeState name (incState state)))))))

;add: adds value into the state
(define add
  (lambda (name value state)
    (if(null? state) 
       (cons (cons name (nameList (initState))) (cons (cons value (valueList (initState))) '()))
       (cons (cons name (nameList (removeState name state))) (cons (cons value (valueList (removeState name state))) '())))))
